...
public class databaze {
	...
	public databaze() 
	{
		...
		prvkyDatabaze=new HashMap<String,kniha>(SQL.getInstance().nacteniKnihZDatabaze());
	...
	public boolean ulozDatabazi(String jmenoSouboru)
	{
		...
		try {
			...
			out.write("Pocet " + prvkyDatabaze.size()); 
			out.newLine();
	...
	public void ulozDatabazi() {
		...
		SQL.getInstance().odstranTabulku();
		SQL.getInstance().vytvorTabulku();
		for (kniha kniha : prvkyDatabaze.values()) {
			SQL.getInstance().vlozKnihu(kniha);
		}
		SQL.getInstance().disconnect();
	}
}